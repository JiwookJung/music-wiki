...
> **구조 변경**: 구 P3(LP 컬렉션)는 별도 프로젝트가 아니라 **이 프로젝트의 LP 수집 모듈(`ingest/lp/`)로 흡수**됨. `lp-collection` repo는 삭제됨. 같은 *앨범* 엔티티를 공유하고 LP↔mp3 연결이 핵심.

## 목표
저장된 mp3 등 음악 + **LP 컬렉션**을 하나의 지식베이스로 정리하고 **LLM 기반 마크다운 위키**(아티스트/앨범/트랙)로 만드는 서비스. 외부 비노출, 로컬용.
...
## 선결정 사항
- 구조: `core/`(공유 데이터모델·위키생성·SQLite) + `ingest/audio/`(P2) + `ingest/lp/`(구 P3, 후순위 마일스톤). 한 덩어리가 아니라 "공유 위키 + 분리된 수집 모듈".
- 데이터 모델: 아티스트/앨범/트랙/레이블/장르/연도. 앨범에 **소장형태 플래그(디지털/바이닐)** → LP↔mp3 연결.
- 저장: **마크다운 위키 `[[링크]]` + SQLite(또는 프론트매터)**로 시작. **neo4j는 v1 보류** — 관계 질의가 실제로 필요해지면 그때 도입.
- 오디오 메타데이터: ID3 태그(아티스트/앨범/트랙/연도/장르) 우선, 부족하면 MusicBrainz 등 외부 보강 + LLM 요약/해설 생성.

## LP 모듈(`ingest/lp/`) 워크플로 (사용자 확정)
- 입력: 선반 한 줄=사진 한 장, 책등 세로 고해상도(샘플 `/mnt/win/memory/음악/lp`, 5472×3648, 4장).
- ① 순서대로(좌→우) **자동 인식**(비전 LLM, 위치+신뢰도) → ② 어렵/실패 항목은 **재촬영 또는 수동 입력**으로 보완 → ③ Discogs/MusicBrainz 정규화 → ④ mp3 라이브러리 대조(바이닐/디지털 보유) → ⑤ 사람 검증 UI로 확정.
- 또렷한 책등은 인식률↑, 얇고 빽빽한 국내반↓ → **부분 카탈로그 + 보완** 전제.
