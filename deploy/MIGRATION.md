...
> **ser8 앞에서 실제로 작업할 때는 [SER8-TASKS.md](SER8-TASKS.md) 체크리스트를 쓰세요.**
> 이 문서는 각 단계의 배경·대안을 설명하는 레퍼런스입니다.
>
> **구조 변경(2026-07-28)**: mp3를 외장 SSD로 ser8에 직결하는 **단독 운영**으로 확정.
> 아래 NFS·WoL 절차는 SSD를 쓰지 않을 때의 폴백으로만 남겨둡니다.
...
| 2 | **외장 SSD 장착·마운트**(`LABEL=MUSIC` → `/mnt/music`) | ser8 | 5분 |
| 3 | `.env` 설정(비밀번호·경로) | ser8 | 2분 |
| 4 | `docker compose up -d` + 적재 3종 | ser8 | 10분 |
| 5 | **동작 확인** — 웹 열기, 재생, 발급 테스트 | ser8 | 5분 |
| 6 | (선택) Tailscale — 외부/폰 접속 | ser8 | 5분 |
| — | ~~NFS 마운트~~ / ~~WoL~~ | — | SSD 직결 시 **불필요**(아래 5·7장은 폴백) |
